...
def get_project_version(version_file):
    version_file = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), version_file
    )
    try:
    ...
    try:
        git_out = subprocess.check_output(
            ["git", "describe", "--always"], stderr=getattr(subprocess, "DEVNULL", None)
        )
    ...
    if not file_ver:
        raise Exception(
            "version not available from git or from file {!r}".format(version_file)
        )
