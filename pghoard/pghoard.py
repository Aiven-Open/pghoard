...
from pghoard.compressor import (
    ...
    CompressionQueue,
    CompressorThread,
    WALFileDeleterThread,
    WalFileDeletionQueue,
)
...
class PGHoard:
    def __init__(self, config_path):
        ...
        self.log.info(
            "pghoard initialized, own_hostname: %r, cwd: %r",
            socket.gethostname(),
            os.getcwd(),
        )
    ...
    def check_pg_versions_ok(self, site, pg_version_server, command):
        ...
        if pg_version_server // 100 != pg_version_client // 100:
            self.log.error(
                "Server version: %r does not match %s version: %r",
                pg_version_server,
                self.config[command + "_path"],
                pg_version_client,
            )
    ...
    def create_basebackup(
        self, site, connection_info, basebackup_path, callback_queue=None, metadata=None
    ):
    ...
    def check_pg_server_version(self, connection_string, site):
        ...
        try:
            ...
            self.log.warning(
                "%s (%s) connecting to DB at: %r",
                ex.__class__.__name__,
                ex,
                connection_string,
            )
            if "password authentication" in str(ex) or "authentication failed" in str(
                ex
            ):
    ...
    def create_backup_site_paths(self, site):
        site_path = os.path.join(
            self.config["backup_location"], self._get_site_prefix(site)
        )
    ...
    def delete_remote_wal_before(self, wal_segment, site, pg_version):
        ...
        while True:
            ...
            wal_path = os.path.join(
                self._get_site_prefix(site), "xlog", lsn.walfile_name
            )
    ...
    def _get_delta_basebackup_files(
        self, site, storage, metadata, basebackup_name_to_delete, backups_to_keep
    ):
        all_hexdigests = set()
        keep_hexdigests = set()
        ...
        delta_backups_to_keep = filter(
            lambda x: x["metadata"]["format"] == BaseBackupFormat.delta_v1,
            backups_to_keep,
        )
        for backup_name in [basebackup_name_to_delete] + [
            back["name"] for back in delta_backups_to_keep
        ]:
            delta_backup_key = os.path.join(
                self._get_site_prefix(site), "basebackup", backup_name
            )
        ...
        for hexdigest in extra_hexdigests:
            basebackup_data_files.append(
                os.path.join(self._get_site_prefix(site), "basebackup_delta", hexdigest)
            )
    ...
    def delete_remote_basebackup(self, site, basebackup, metadata, basebackups):
        ...
        main_backup_key = os.path.join(
            self._get_site_prefix(site), "basebackup", basebackup
        )
        ...
        if metadata.get("format") == BaseBackupFormat.v2:
            ...
            basebackup_data_files.extend(
                self._get_delta_basebackup_files(
                    site, storage, metadata, basebackup, basebackups
                )
            )

        self.log.debug(
            "Deleting basebackup datafiles: %r", ", ".join(basebackup_data_files)
        )
        ...
        self.log.info(
            "Deleted basebackup datafiles: %r, took: %.2fs",
    ...
    def patch_basebackup_info(self, *, entry, site_config):
        ...
        if "normalized-backup-time" not in metadata:
            metadata["normalized-backup-time"] = self.get_normalized_backup_time(
    ...
    def determine_backups_to_delete(self, *, basebackups, site_config):
        ...
        backup_interval = datetime.timedelta(
            hours=basebackup_interval_hours if basebackup_interval_hours else 0
        )
        ...
        if max_age_days and min_backups > 0:
            while basebackups and len(basebackups) > min_backups:
                ...
                completed_at = (
                    basebackups[0]["metadata"]["start-time"] + backup_interval
                )
    ...
    def get_normalized_backup_time(self, site_config, *, now=None):
        ...
        if (
            backup_hour is None
            or backup_minute is None
            or backup_interval_hours is None
        ):
            return None
        ...
        if normalized.hour < backup_hour or (
            normalized.hour == backup_hour and normalized.minute < backup_minute
        ):
            ...
            for filename in os.listdir(uncompressed_xlog_path):
                ...
                if wal.PARTIAL_WAL_RE.match(filename):
                    ...
                    if file_size in {0, wal.WAL_SEG_SIZE}:
                        self.log.info(
                            "Found partial file %r, size %d bytes", full_path, file_size
                        )
                    else:
                        self.log.warning(
                ...
                if not wal.WAL_RE.match(filename) and not wal.TIMELINE_RE.match(
                    filename
                ):
                    self.log.warning(
                        "Found invalid file %r from incoming xlog directory", full_path
                    )
                    continue

                filetype = (
                    FileType.Timeline
                    if wal.TIMELINE_RE.match(filename)
                    else FileType.Wal
                )
    ...
    def handle_site(self, site, site_config):
        ...
        if site not in self.receivexlogs and site not in self.walreceivers:
            if site_config["active_backup_mode"] == "pg_receivexlog":
                self.receivexlog_listener(
                    site, chosen_backup_node, xlog_path + "_incoming"
                )
                ...
                with suppress(FileNotFoundError):
                    with open(state_file_path, "r") as fp:
                        ...
                        walreceiver_state = old_state_file.get("walreceivers", {}).get(
                            site, {}
                        )
        ...
        if site in self.basebackups:
            ...
            if (
                not last_check_time
                or (time.monotonic() - self.time_of_last_backup_check[site]) > 300
            ):
                self.refresh_backup_list_and_delete_old(site)
                self.time_of_last_backup_check[site] = time.monotonic()
        ...
        if metadata and not os.path.exists(self.config["maintenance_mode_file"]):
            if site in self.delta_backup_failures:
                ...
                if retries > site_config["basebackup_delta_mode_max_retries"]:
                    self.log.info(
                        "Giving up backup after exceeding max retries: %r", retries
                    )
                    return
                else:
                    ...
                    if utc_now() >= self.delta_backup_failures[
                        site
                    ].last_failed_time + datetime.timedelta(seconds=retry_interval):
    ...
    def get_new_backup_details(self, *, now=None, site, site_config):
        ...
        if site in self.requested_basebackup_sites:
            ...
            normalized_time_changed = (
                last_normalized_backup_time != normalized_backup_time
            )
            last_scheduled_isnt_too_recent = (
                not most_recent_scheduled or most_recent_scheduled + delta <= now
            )
            ...
            self.log.warning(
                "Ignoring basebackup_hour as basebackup_minute is not defined"
            )
        else:
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                self.log.error(
                    "main loop: %s: %s, retrying...", ex.__class__.__name__, ex
                )
            ...
            if self.thread_critical_failure_event.wait(timeout=5.0):
                self.log.error(
                    "Unexpected critical failure in PGHoard thread. Quitting main loop."
                )
                self.quit()
    ...
    def write_backup_state_to_json_file(self):
        ...
        self.state["served_files"] = (
            self.webserver.get_most_recently_served_files() if self.webserver else {}
        )
        self.log.debug("Writing JSON state file to %r", state_file_path)
        write_json_file(state_file_path, self.state)
        self.log.debug(
            "Wrote JSON state file to disk, took %.4fs", time.time() - start_time
        )

    def load_config(self, _signal=None, _frame=None):  # pylint: disable=unused-argument
        self.log.debug(
            "Loading JSON config from: %r, signal: %r", self.config_path, _signal
        )
        try:
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        prog="pghoard", description="postgresql automatic backup daemon"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-s", "--short-log", help="use non-verbose logging format", action="store_true"
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "config_file",
        help="configuration file path (for backward compatibility)",
        nargs="?",
    )
    ...
    logutil.configure_logging(
        short_log=arg.short_log, level=logging.DEBUG if arg.debug else logging.INFO
    )
