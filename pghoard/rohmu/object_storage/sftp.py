...
class SFTPTransfer(BaseTransfer):
    def __init__(self,
        ...
        # https://bitbucket.org/dundeemt/pysftp/issues/109/hostkeysexception-no-host-keys-found-even
        warnings.filterwarnings('ignore', '.*hostkeys = None*',)
        self.cnopts = pysftp.CnOpts()
        self.cnopts.hostkeys = None

        self.cnopts.log = False
        logging.getLogger("paramiko").setLevel(logging.WARNING)

        self.client = pysftp.Connection(self.server,
                                        port=self.port,
    ...
    def _get_contents_to_fileobj(self, key, fileobj_to_store_to, progress_callback=None):
        ...
        try:
            # the pysftp progress callback has the same interface as pghoard for downloads
            return self.client.getfo(target_path, fileobj_to_store_to, progress_callback)
    ...
    def store_file_object(self, key, fd, *, cache_control=None, metadata=None, mimetype=None, upload_progress_fn=None):
        ...
        self.client.makedirs(os.path.dirname(target_path))
        self.client.putfo(flo=fd, remotepath=target_path, callback=wrapper_upload_progress_fn)
    ...
    def _save_metadata(self, target_path, metadata):
        ...
        self.client.putfo(flo=bio, remotepath=metadata_path)
