...
def create_keys(bits):
    ...
    return rsa_private_key_pem_bin.decode("ascii"), rsa_public_key_pem_bin.decode(
        "ascii"
    )
...
def save_keys(config_file, site, key_id, rsa_private_key, rsa_public_key):
    config_obj = config.read_json_config_file(
        config_file, check_commands=False, add_defaults=False
    )
    ...
    print(
        "Saved new key_id {!r} for site {!r} in {!r}".format(key_id, site, config_file)
    )
    print(
...
def main():
    ...
    parser.add_argument(
        "--key-id",
        help="key alias as used with encryption_key_id configuration directive",
        required=True,
    )
    parser.add_argument(
        "--bits",
        help="length of the generated key in bits, default %(default)d",
        default=3072,
        type=int,
    )
    parser.add_argument(
        "--config",
        help="configuration file to store the keys in",
    ...
    try:
        if args.config:
            return save_keys(
                args.config, args.site, args.key_id, rsa_private_key, rsa_public_key
            )
        else:
            return show_key_config(
                args.site, args.key_id, rsa_private_key, rsa_public_key
            )
