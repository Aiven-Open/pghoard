...
def test_check_wal_archive_integrity(requests_put_mock, requests_head_mock, tmpdir):
    ...
    write_json_file(
        ...
        {
            "http_port": 8080,
            "backup_sites": {"foo": {"pg_data_directory": pg_data_directory}},
        },
    )
    arsy = ArchiveSync()
...
def test_check_and_upload_missing_local_files(
    requests_put_mock, requests_head_mock, tmpdir
):
    ...
    write_json_file(
        config_file,
        {"http_port": 8080, "backup_sites": {"foo": {"pg_data_directory": data_dir}}},
    )
    arsy = ArchiveSync()
    arsy.set_config(config_file, site="foo")
